...
class _GraphAreaState extends State<GraphArea>
  ...
  void initState() {
    ...
    generateData(widget
        .graphType); // Initial data generation, can be changed according to your needs
  }
  ...
  void generateData(String type) {
    setState(() {
      switch (type) {
        case 'Weekly':
          /*data = [
            DataPoint(day: daysOfWeek[1]!, steps: Random().nextInt(10000)),
          ...
          data = List.generate(
            7,
            (index) => DataPoint(
                day: daysOfWeek[index + 1]!, steps: Random().nextInt(10000)),
          );
          dotDay = globalDateBox.nowweekday;

          break;

        case 'Monthly':
          data = List.generate(
            globalDateBox.daysinmonth,
            (index) => DataPoint(
                day: (index + 1).toString(), steps: Random().nextInt(10000)),
          );
          dotDay = globalDateBox.nowday;
          break;
...
class GraphPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < dotDay; i++) {
      // Loop only up to dotDay
    ...
    if (data.length > 25) {
      List<String> showOnlyDays = [
        ...
        '25',
        '30'
      ]; // Define the days to be displayed
      for (int i = 0; i < data.length; i++) {
        if (!showOnlyDays.contains(data[i].day))
          continue; // Skip days not in the list
        var x = i * xSpacing;
        canvas.drawLine(
    ...
    canvas.drawCircle(
        offsets[dotDay - 1], 15 * _dotSize.value, dotOutlinePaint);
    canvas.drawCircle(offsets[dotDay - 1], 6 * _dotSize.value, dotCenter);
  }
